...
import { migrateAccount, MAX_SOURCE_ACCOUNTS } from "./migrate.ts";
...
server.registerTool(
  "migrate_account",
  {
    ...
    description:
      ...
      "with `import-state.ts <state.json> --account <name>`), so cookies never pass through the " +
    ...
    inputSchema: {
      sourceAccounts: z
        ...
        .describe(
          `Names of 1..${MAX_SOURCE_ACCOUNTS} source accounts to merge (files under ` +
            "~/.cookidoo-mcp/accounts/). Generic over count: 1→1, 2→1, … N→1 all supported.",
        ),
