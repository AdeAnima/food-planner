...
// `contentType` defaults to JSON (body is JSON.stringify'd); pass
// "application/x-www-form-urlencoded" + a string body to send a form-encoded request
// verbatim (the /me endpoint's native form-submit shape — see updateProfile fallback).
...
// 10c — rename a collection. Same PUT endpoint as addRecipesToCollection, which is a blind
// full-replace of membership — a {title}-only PUT would WIPE recipeIds. So GET-merge-PUT:
// read current membership, then PUT {title, recipeIds: current} (raw server ids, never
// re-normalized — same discipline as 10a). Live-verified web+cookie 2026-06-23: title-in-PUT
// accepted (200) and membership preserved. No vendor Accept (response body discarded).
export async function renameCollection(
  ...
  await authedWrite(
    ...
    "PUT",
    `${BASE}/organize/${PATH_LOCALE}/api/custom-list/${encodeURIComponent(id)}`,
    { title, recipeIds: current.recipeIds },
    `${BASE}/organize/${PATH_LOCALE}`,
  );
  return { id: current.id, title, total: current.recipeIds.length };
}
...
export interface Profile {
  id: string;
  username?: string;
  picture?: string; // full CDN avatar URL (userInfo.picture); undefined if unset
  isPublic?: boolean;
...
export async function getProfile(): Promise<Profile> {
  const body = (await authedRead("profile fetch", `${BASE}/community/profile`)) as {
    ...
    isPublic?: boolean;
    foodPreferences?: string[];
    userInfo?: { username?: string; picture?: string };
    thermomixes?: unknown[];
  };
  return {
    id: body.id ?? "",
    username: body.userInfo?.username || undefined,
    picture: body.userInfo?.picture || undefined,
...
// PUT /community/profile/{PATH_LOCALE}/food-preferences — true PUT (no _method). Body is the
// FULL set of currently-checked values (vegetarian|easy|creative|traditional), not a delta.
export async function setFoodPreferences(foodPreferences: string[]): Promise<unknown> {
  return authedWrite(
    "set food preferences",
    "PUT",
    `${BASE}/community/profile/${PATH_LOCALE}/food-preferences`,
    { foodPreferences },
  );
}

// PUT /community/profile/{PATH_LOCALE}/me — username (≤40 chars) and/or avatar picture (full CDN
// URL; "" clears it). Only the provided fields are sent; with neither, it's a no-op (no request).
//
// Shape-robust: the UI submits this as a NATIVE FORM (urlencoded, hidden _method=put). The
// JSON+_method shape (mirroring rateRecipe) is INFERRED, never live-captured, so try it first
// and fall back to the captured urlencoded form-submit on a 4xx — avoids needing a live throwaway
// account just to de-risk one body shape. ponytail: two shapes tried, not one captured-and-trusted,
// because this is a live mutating boundary where a wrong guess silently no-ops the identity write.
export async function updateProfile(fields: { username?: string; picture?: string }): Promise<unknown> {
  const url = `${BASE}/community/profile/${PATH_LOCALE}/me`;
  const referer = `${BASE}/community/profile/${PATH_LOCALE}`;
  const provided: Record<string, string> = {};
  if (fields.username !== undefined) provided.username = fields.username;
  if (fields.picture !== undefined) provided.picture = fields.picture;
  if (Object.keys(provided).length === 0) return { ok: true };

  try {
    return await authedWrite("update profile", "PUT", url, { _method: "put", ...provided }, referer);
  } catch (e) {
    // Only fall back on a client-side rejection (4xx = wrong shape); re-throw auth/5xx/network.
    if (!(e instanceof Error) || !/failed: 4\d\d/.test(e.message)) throw e;
    const form = new URLSearchParams({ _method: "put", ...provided }).toString();
    return authedWrite("update profile", "POST", url, form, referer, "application/json", "application/x-www-form-urlencoded");
  }
}
